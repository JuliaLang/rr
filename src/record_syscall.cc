...
static bool os_has_broken_zfs()
{
  ...
  ScopedFd version_file_fd = ScopedFd(openat(dirfd(zfs_dir), "version", O_RDONLY));
  closedir(zfs_dir);
  char version[50];
  ...
  if (3 != sscanf(version, "%d.%d.%d", &zfs_major, &zfs_minor, &zfs_patch)) {
    LOG(warn) << "Failed to parse /sys/module/zfs/version; assuming ZFS is broken";
    return true;
  }
  is_broken = !(zfs_major > 2 || zfs_minor > 2);
  return is_broken;
}

// On some versions of ZFS, SEEK_HOLE/SEEK_DATA is known to not be reliable
// and potentially return spurious holes. Such holes would cause trace corruption,
// so we must skip this optimizations on such ZFS versions.
static bool may_have_zfs_seek_bug(ScopedFd &fd)
{
  ...
  if (bad_file && !did_warn) {
    LOG(warn) << "Detected access to file on ZFS < 2.3.0 which has known bugs."
